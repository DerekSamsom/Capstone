...
patient_distance is right-skewed, therefore NA values will be replaced with
...
First I will check for negative values.  Negative values suggest the appointment occured before it was scheduled, which wouldn't be possible and must represent an entry error.
...
Looking at `appt_lead_time` proportionally, the drop in missed appointments among those with the shortest lead tims is easier to see.
...
The weekday the appointments are scheduled follows a similar pattern as the weekday the appointments occur, peaking on Tuesday and trailing off as the week progresses.  However, the weekday the appointment is made has the opposite effect as the weekday the appointment occurst, with the ratio of missed appointments gradually rising throughout the week.
