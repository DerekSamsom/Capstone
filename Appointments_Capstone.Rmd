...
```{r}
appointments_2 <- appointments_2 %>% 
    mutate(length_group = cut(appt_length, breaks = c(-1, 45, 75, 1000), labels = c("Short", "Medium", "Long")))
...
```{r eval = FALSE}
rftrain <- caret::train(kept_status ~ patient_age + remind_call_result + provider_specialty + billing_type + hour + percent_missed + appt_length + patient_gender + patient_distance + new + appt_lead_time + weekday + county_code + city_size, data = train_balanced, method = "rf", metric = metric, tuneGrid = tunegrid, trControl = control)
```
